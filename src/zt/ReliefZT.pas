...
begin
  ...
  begin
    if (Application.MessageBox(PChar('Načítáte soubor s verzí ' + ver + #13#10 + 'Aplikace momentálně podporuje verzi '
      + _ZT_FileVersion + #13#10 + 'Chcete pokračovat?'), 'Varování', MB_YESNO OR MB_ICONQUESTION) = mrNo) then
    begin
      Result := 2;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    for j := 0 to Self.Useky[i].Vetve.Count - 1 do
    begin
      obj := Format('%.3d%.2d%.2d', [Self.Useky[i].Vetve.Data[j].vyh, Self.Useky[i].Vetve.Data[j].ref_plus,
        Self.Useky[i].Vetve.Data[j].ref_minus]);
      for k := 0 to Length(Self.Useky[i].Vetve.Data[j].Symbols) - 1 do
        obj := obj + Format('%.3d%.3d%.3d', [Self.Useky[i].Vetve.Data[j].Symbols[k].Position.X,
          Self.Useky[i].Vetve.Data[j].Symbols[k].Position.Y, Self.Useky[i].Vetve.Data[j].Symbols[k].SymbolID]);
    ...
    obj := '';
    for j := 0 to Self.Useky[i].JCClick.Count - 1 do
      obj := obj + Format('%.3d%.3d', [Self.Useky[i].JCClick.Data[j].X, Self.Useky[i].JCClick.Data[j].Y]);
    inifile.WriteString('U' + IntToStr(i), 'C', obj);

    // KPopisek
    obj := '';
    for j := 0 to Self.Useky[i].KPopisek.Count - 1 do
      obj := obj + Format('%.3d%.3d', [Self.Useky[i].KPopisek.Data[j].X, Self.Useky[i].KPopisek.Data[j].Y]);
  ...
  begin
    ...
    obj := '';
    for j := 0 to Length(Self.PomocneObj[i].Pos) - 1 do
      obj := obj + Format('%.3d%.3d', [Self.PomocneObj[i].Pos[j].X, Self.PomocneObj[i].Pos[j].Y]);
...
begin
  ...
  begin
    line := Self.JCs.Data[i].Nazev + '|' + IntToStr(Integer(Self.JCs.Data[i].typ)) + '|' + Self.JCs.Data[i].poznamka +
      '|' + IntToStr(Self.JCs.Data[i].nav) + '|' + Self.JCs.Data[i].prejezdy + '|' +
      IntToStr(Self.JCs.Data[i].trat.index) + '|' + IntToStr(Self.JCs.Data[i].trat.smer) + '|' +
      IntToStr(Self.JCs.Data[i].dalsiNav) + '|' + IntToStr(Self.JCs.Data[i].rychlost) + '|';
    ...
    for j := 0 to Length(Self.JCs.Data[i].vyhybky) - 1 do
      line := line + IntToStr(Self.JCs.Data[i].vyhybky[j].Blok) + ',' +
        IntToStr(Self.JCs.Data[i].vyhybky[j].poloha) + ';';
    line[Length(line)] := '|';

    // odvraty
    for j := 0 to Length(Self.JCs.Data[i].odvraty) - 1 do
      line := line + IntToStr(Self.JCs.Data[i].odvraty[j].Blok) + ',' +
        IntToStr(Self.JCs.Data[i].odvraty[j].poloha) + ';';
    line[Length(line)] := '|';
